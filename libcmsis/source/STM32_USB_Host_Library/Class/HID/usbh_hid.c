...
static USBH_StatusTypeDef USBH_HID_InterfaceInit(USBH_HandleTypeDef *phost) {
	...
	interface = USBH_FindInterface(phost, phost->pActiveClass->ClassCode, HID_BOOT_CODE,
			0xFF);
...
static USBH_StatusTypeDef USBH_HID_SetHIDOutput(USBH_HandleTypeDef *phost)
{
	...
	USBH_LL_SetToggle(phost, HID_Handle->OutPipe, 0);
	USBH_InterruptSendData(phost, buff, 1, HID_Handle->OutPipe);
	osEvent event = osMessagePurge(phost->os_event,0x03);
	if(event.status == osEventMessage )
	{
		USBH_StatusTypeDef URB_Status = USBH_LL_GetURBState(phost, HID_Handle->OutPipe);
...
static USBH_StatusTypeDef HID_SetIdle(USBH_HandleTypeDef *phost, uint8_t duration,
		uint8_t reportId)
{

	USBH_StatusTypeDef ret = USBH_FAIL;
	osEvent event;
	...
	for(;; )
	{
		event = osMessagePurge(phost->os_event, CTRL_WAIT_TIME );

		if( event.status == osEventMessage )
		{
			ret = USBH_CtlReq(phost, 0, 0);
			if( (ret == USBH_OK) || (USBH_NOT_SUPPORTED == ret))
			{
				break;
			}
		}
		//if disconnect or device error;
		else
		{
			break;
		}
...
static USBH_StatusTypeDef HID_SetProtocol(USBH_HandleTypeDef *phost, uint8_t protocol)
{
	USBH_StatusTypeDef ret = USBH_FAIL;
	osEvent event;
...
static USBH_StatusTypeDef HID_SetReport(USBH_HandleTypeDef *phost,
		...
		uint8_t reportLen)
{
	...
	for(;; )
	{
		...
		if( event.status == osEventMessage )
		{
			ret = USBH_CtlReq(phost, reportBuff, reportLen);
			if( (ret == USBH_OK) || (USBH_NOT_SUPPORTED == ret) )
			{
				break;
			}
...
static USBH_StatusTypeDef HID_GetReport(USBH_HandleTypeDef *phost,
		uint8_t reportType, uint8_t reportId, uint8_t* reportBuff,
		uint8_t reportLen)
{
	USBH_StatusTypeDef ret = USBH_FAIL;
...
static USBH_StatusTypeDef USBH_HID_ClassRequest(USBH_HandleTypeDef *phost) {
	...
	HID_HandleTypeDef *HID_Handle =
			(HID_HandleTypeDef *) phost->pActiveClass->pData;
	//ignore the cmd when subclass != HID boot;
	if (HID_REQ_INIT == HID_Handle->ctl_state) {
		if (phost->device.CfgDesc.Itf_Desc[0].bInterfaceSubClass
				!= HID_BOOT_CODE)
	...
	case HID_REQ_GET_REPORT_DESC:
		...
		if (USBH_HID_GetHIDReportDescriptor(phost,
				HID_Handle->HID_Desc.wItemLength) == USBH_OK) {
	...
	case HID_REQ_SET_IDLE:
		...
		classReqStatus = HID_SetIdle(phost, 0, 0);
		/* set Idle */
...
//		if (USBH_HID_SetProtocol(phost, 0) == USBH_OK)
		if(HID_SetProtocol(phost, 0) == USBH_OK)
		{
			HID_Handle->ctl_state = HID_REQ_SET_REPORT;
			eventID = 400;
			osMessagePut(phost->os_event, eventID, 0);
	...
	{
		uint8_t buff[2] = { 0x0, 0x0 };
		//USBH_StatusTypeDef ret = USBH_HID_SetReport(phost, 2, 0, buff, 1);
		USBH_StatusTypeDef ret = HID_SetReport(phost, 2, 0, buff, 1);
...
static USBH_StatusTypeDef USBH_HID_Process(USBH_HandleTypeDef *phost) {
	...
	case HID_IDLE:
		if (USBH_SPEED_LOW == phost->device.speed)
		{
		...
		{
			//statusHid = USBH_HID_GetReport(phost, 0x01, 0, HID_Handle->pData,
			//		HID_Handle->length);
			statusHid = HID_GetReport(phost, 0x01, 0, HID_Handle->pData,
								HID_Handle->length);
			if (statusHid == USBH_OK)
			{
				fifo_write(&HID_Handle->fifo, HID_Handle->pData,
						HID_Handle->length);
				HID_Handle->state = HID_SYNC;
			} else
			{
